...
The attestor service will communicate directly with our node therefore, make sure that your Pathfinder node is already running and it's synced to the latest block. 
Depending if you are using Juno or Pathfinder node use the respective attestation tool 
